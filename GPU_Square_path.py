...
box2 = PrimitiveMessage(
    ...
    materialID=TRANSPARENT_BLACK,
    pose=Pose(
        position=Vector3(1.5, box_height/4,-1.5),
...
box4 = PrimitiveMessage(
    ...
    materialID=TRANSPARENT_BLACK,
    pose=Pose(
        position=Vector3(0, box_height/4,-3),
        rotation=Quaternion(1, 0, 0, 1),
        scale=Vector3(box_length, box_width, box_height/2)  # Scale appropriately for rectangular box
    ),
    isManipulationOn=False,
...
positions = cp.array([[random.uniform((-box_length/2)+.1, (-box_length/4)-.1), 
...
force_vector_2 = cp.array([-0.03, 0.0, -0.05])  # Adjust the magnitude as needed
...
outer_box = {'x': (-box_length/2, box_length/2), 'y': (0, 0.45), 'z': (-box_length+ (box_width/2), (box_width/2))}
inner_box = {'a': ((-box_length/4)-radius_particle, (box_length/4)+radius_particle), 'c': ((-box_length+ (3*box_width/2))-radius_particle, (-box_width/2)+radius_particle)}
